...
subanalysis_do   = False # Turns on/off the coloring and separate R calculation of data based on a clinical parameter
log_scale        = False
...
def WaterfallPlot(dictionary, target_gene, gene_set):
	...
	if len(highlight_genes) != 0:
		...
		for rank, r_value in zip(highlight_ranks, highlight_r_values):
			plt.vlines(x=rank, ymin=min(r_values), ymax=max(r_values), color='red', linewidth=0.2, zorder=1, label= '%s (%.0f, p=%f)' %(name_gene_set, stat, p_value))
...
gene_sets = {
    ...
    'REACTOME_CHOLESTEROL_BIOSYNTHESIS' : ['CYP51A1', 'MSMO1', 'IDI1', 'SREBF1', 'FDFT1', 'SQLE', 'SC5D','MVK','HMGCS1','HMGCR','DHCR24','ACAT2','HSD17B7','LBR','EBP','NSDHL','IDI2','TM7SF2','GGPS1','LSS','FDPS','PMVK','MVD','DHCR7','ARV1','SREBF2', 'PLPP6', 'FDFT1']
    }
 
# This function helps define gene sets to be highlighted.
# A set of genes is selected based on the string used as input.
# First, the string is tested in the custom set of gene sets in gene_sets above.
# If that fails, it will use the string to search for a public gene set on Msigdb
# If that fails, it will default interpreting the input string as a set with a single gene name (the input string)
...
gene_set = GetGeneSet(name_gene_set)
